...
public class RequestGenTool {
	private static Logger logger = LoggerFactory.getLogger(RequestGenTool.class);
...
//	private static final String PATH = "C:\\javaProject\\jdbc-template\\src\\main\\java\\test\\";
	...
	private static final String CLASS_PREFIX = "Req";
	private static final String CLASS_TARGET_SUFFIX = "";	
	private static final String CFG_ROOT = "gen";
	private static final String FREE_MARK_TEMPLATE = "as/req.ftl";
	...
	public static void main(String[] args) throws Exception {
		...
		RequestGenTool.getFreeMarkerCFG(CFG_ROOT);

		Map<String, Object> map = collectBeans();

		RequestGenTool.beans.clear();
		for (Map.Entry<String, Object> entry : map.entrySet()) {
			if (entry.getKey().indexOf(PACKAGE) >= 0) {
				RequestGenTool.reflectObj(entry.getValue());
			}
		}
		RequestGenTool.genFiles();
	}
	...
	public static void reflectObj(Object obj) {
		...
		for (Field f : fields) {
			...
			if (f.getType().isArray()) {
				if(f.getType().getSimpleName().indexOf("byte")>=0){
					bean.addMethod(f.getType().getSimpleName(), f.getName(), desc);
				}else{
