...
public class RequestService {
	...
	public void processGeneralRequest(GeneralRequest request, BaseConnection conn) {		
		...
		logger.debug(">>>>> request:{} ,session:{}" , request.getCmd() , conn.getSessionId());
		
		long beginTime = System.currentTimeMillis();
		RequestUtils.setCurrentConn(conn);
		handlerService.handle(request, conn);
		RequestUtils.doPenddingJobs();
		RequestUtils.setCurrentConn(null);	
		
		//if(logger.isDebugEnabled())
		logger.debug("<<<<< cmd:{}, spend:{}ms", request.getCmd(), (System.currentTimeMillis() - beginTime));
	}
