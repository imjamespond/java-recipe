 ...
 * @author fangyaoxia
 */
@Repository(value = "playerActionModelDao")
public class PlayerActionModelDaoImpl implements IPlayerActionModelDao{
	private static final Log log = LogFactory.getLog(PlayerActionModelDaoImpl.class);

    @Autowired
    private HibernateTemplate template;

	@Override
	public void createBean(PlayerActionModel playerActionModel) throws BeanAreadyException{
		if (log.isDebugEnabled()){
			log.debug("createBean:"+playerActionModel);
		}
		try{
			findById(playerActionModel.getId());
		}catch(NotFoundBeanException e){
            template.save(playerActionModel);
		}		
	}
	...
	public void updateBean(PlayerActionModel playerActionModel) throws NotFoundBeanException{
		...
		findById(playerActionModel.getId());
        template.update(playerActionModel);
	}
	...
	public void removeBean(PlayerActionModel playerActionModel) throws NotFoundBeanException{
		...
		findById(playerActionModel.getId());
        template.delete(playerActionModel);
	}
	...
	public void removeBean(Serializable id) throws NotFoundBeanException{
        ...
        PlayerActionModel playerActionModel = findById(id);
        template.delete(playerActionModel);
	}
	...
	public PlayerActionModel findById(Serializable id) throws NotFoundBeanException{
		...
		PlayerActionModel playerActionModel = (PlayerActionModel)template.get(PlayerActionModel.class, id);;
		if (null==playerActionModel)
	...
	public List<PlayerActionModel> findByNameQuery(String nameQuery,Map<String,Object> params){
        ...
        List <PlayerActionModel> list = (List<PlayerActionModel>)template.findByNamedParam(nameQuery, keys.toArray(new String[0]),values.toArray(new Object[0]));
        return list;
	}

                @SuppressWarnings("unchecked")
                @Override
                public List<PlayerActionModel> findPages(final String nameQuery, final Map<String, Object> params, final int begin, final int offset) {
                    if (log.isDebugEnabled()) {
                    log.debug("findPages:[begin=" + begin + ",offset=" + offset + ",nameQuery=" + nameQuery + "]");
                    }
                    List<PlayerActionModel> list = (List<PlayerActionModel>) template.execute(new HibernateCallback
                        <Object>() {
                            @Override
