 ...
 * @author fangyaoxia
 */
@Repository(value = "playerActivityActionModelDao")
public class PlayerActivityActionModelDaoImpl implements IPlayerActivityActionModelDao{
	private static final Log log = LogFactory.getLog(PlayerActivityActionModelDaoImpl.class);

    @Autowired
    private HibernateTemplate template;

	@Override
	public void createBean(PlayerActivityActionModel playerActivityActionModel) throws BeanAreadyException{
		if (log.isDebugEnabled()){
			log.debug("createBean:"+playerActivityActionModel);
		}
		try{
			findById(playerActivityActionModel.getId());
		}catch(NotFoundBeanException e){
            template.save(playerActivityActionModel);
		}		
	}
	...
	public void updateBean(PlayerActivityActionModel playerActivityActionModel) throws NotFoundBeanException{
		...
		findById(playerActivityActionModel.getId());
        template.update(playerActivityActionModel);
	}
	...
	public void removeBean(PlayerActivityActionModel playerActivityActionModel) throws NotFoundBeanException{
		...
		findById(playerActivityActionModel.getId());
        template.delete(playerActivityActionModel);
	}
	...
	public void removeBean(Serializable id) throws NotFoundBeanException{
        ...
        PlayerActivityActionModel playerActivityActionModel = findById(id);
        template.delete(playerActivityActionModel);
	}
	...
	public PlayerActivityActionModel findById(Serializable id) throws NotFoundBeanException{
		...
		PlayerActivityActionModel playerActivityActionModel = (PlayerActivityActionModel)template.get(PlayerActivityActionModel.class, id);
		if (null==playerActivityActionModel)
	...
	@Override
	public List<PlayerActivityActionModel> findByNameQuery(String nameQuery,Map<String,Object> params){
		if (log.isDebugEnabled()){
        ...
        List <PlayerActivityActionModel> list = (List<PlayerActivityActionModel>)template.findByNamedParam(nameQuery, keys.toArray(new String[0]),values.toArray(new Object[0]));
        return list;
	}

                @SuppressWarnings("unchecked")
                @Override
                public List<PlayerActivityActionModel> findPages(final String nameQuery, final Map<String, Object> params, final int begin, final int offset) {
                    if (log.isDebugEnabled()) {
                    log.debug("findPages:[begin=" + begin + ",offset=" + offset + ",nameQuery=" + nameQuery + "]");
                    }
                    List<PlayerActivityActionModel> list = (List<PlayerActivityActionModel>) template.execute(new HibernateCallback
                        <Object>() {
                            @Override
