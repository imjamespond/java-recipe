 ...
 * @author fangyaoxia
 */
@Repository(value = "playerOrderResetActionModelDao")
public class PlayerOrderResetActionModelDaoImpl implements IPlayerOrderResetActionModelDao{
	private static final Log log = LogFactory.getLog(PlayerOrderResetActionModelDaoImpl.class);

    @Autowired
    private HibernateTemplate template;

	@Override
	public void createBean(PlayerOrderResetActionModel playerOrderResetActionModel) throws BeanAreadyException{
		if (log.isDebugEnabled()){
			log.debug("createBean:"+playerOrderResetActionModel);
		}
		try{
			findById(playerOrderResetActionModel.getId());
		}catch(NotFoundBeanException e){
            template.save(playerOrderResetActionModel);
		}		
	}
	...
	public void updateBean(PlayerOrderResetActionModel playerOrderResetActionModel) throws NotFoundBeanException{
		...
		findById(playerOrderResetActionModel.getId());
        template.update(playerOrderResetActionModel);
	}
	...
	public void removeBean(PlayerOrderResetActionModel playerOrderResetActionModel) throws NotFoundBeanException{
		...
		findById(playerOrderResetActionModel.getId());
        template.delete(playerOrderResetActionModel);
	}
	...
	public void removeBean(Serializable id) throws NotFoundBeanException{
        ...
        PlayerOrderResetActionModel playerOrderResetActionModel = findById(id);
        template.delete(playerOrderResetActionModel);
	}
	...
	public PlayerOrderResetActionModel findById(Serializable id) throws NotFoundBeanException{
		...
		PlayerOrderResetActionModel playerOrderResetActionModel = (PlayerOrderResetActionModel)template.get(PlayerOrderResetActionModel.class, id);;
		if (null==playerOrderResetActionModel)
	...
	@Override
	public List<PlayerOrderResetActionModel> findByNameQuery(String nameQuery,Map<String,Object> params){
		if (log.isDebugEnabled()){
        ...
        List <PlayerOrderResetActionModel> list = (List<PlayerOrderResetActionModel>)template.findByNamedParam(nameQuery, keys.toArray(new String[0]),values.toArray(new Object[0]));
        return list;
	}

                @SuppressWarnings("unchecked")
                @Override
                public List<PlayerOrderResetActionModel> findPages(final String nameQuery, final Map<String, Object> params, final int begin, final int offset) {
                    if (log.isDebugEnabled()) {
                    log.debug("findPages:[begin=" + begin + ",offset=" + offset + ",nameQuery=" + nameQuery + "]");
                    }
                    List<PlayerOrderResetActionModel> list = (List<PlayerOrderResetActionModel>) template.execute(new HibernateCallback
                        <Object>() {
                            @Override
