 ...
 * @author fangyaoxia
 */
@Repository(value = "playerRechargeActionModelDao")
public class PlayerRechargeActionModelDaoImpl implements IPlayerRechargeActionModelDao{
	private static final Log log = LogFactory.getLog(PlayerRechargeActionModelDaoImpl.class);

    @Autowired
    private HibernateTemplate template;

	@Override
	public void createBean(PlayerRechargeActionModel playerRechargeActionModel) throws BeanAreadyException{
		if (log.isDebugEnabled()){
			log.debug("createBean:"+playerRechargeActionModel);
		}
		try{
			findById(playerRechargeActionModel.getId());
		}catch(NotFoundBeanException e){
            template.save(playerRechargeActionModel);
		}		
	}
	...
	public void updateBean(PlayerRechargeActionModel playerRechargeActionModel) throws NotFoundBeanException{
		...
		findById(playerRechargeActionModel.getId());
        template.update(playerRechargeActionModel);
	}
	...
	public void removeBean(PlayerRechargeActionModel playerRechargeActionModel) throws NotFoundBeanException{
		...
		findById(playerRechargeActionModel.getId());
        template.delete(playerRechargeActionModel);
	}
	...
	public void removeBean(Serializable id) throws NotFoundBeanException{
        ...
        PlayerRechargeActionModel playerRechargeActionModel = findById(id);
        template.delete(playerRechargeActionModel);
	}
	...
	public PlayerRechargeActionModel findById(Serializable id) throws NotFoundBeanException{
		...
		PlayerRechargeActionModel playerRechargeActionModel = (PlayerRechargeActionModel)template.get(PlayerRechargeActionModel.class, id);;
		if (null==playerRechargeActionModel)
	...
	@Override
	public List<PlayerRechargeActionModel> findByNameQuery(String nameQuery,Map<String,Object> params){
		if (log.isDebugEnabled()){
        ...
        List <PlayerRechargeActionModel> list = (List<PlayerRechargeActionModel>)template.findByNamedParam(nameQuery, keys.toArray(new String[0]),values.toArray(new Object[0]));
        return list;
	}

                @SuppressWarnings("unchecked")
                @Override
                public List<PlayerRechargeActionModel> findPages(final String nameQuery, final Map<String, Object> params, final int begin, final int offset) {
                    if (log.isDebugEnabled()) {
                    log.debug("findPages:[begin=" + begin + ",offset=" + offset + ",nameQuery=" + nameQuery + "]");
                    }
                    List<PlayerRechargeActionModel> list = (List<PlayerRechargeActionModel>) template.execute(new HibernateCallback
                        <Object>() {
                            @Override
