 ...
 * @author fangyaoxia
 */
@Repository(value = "playerRegisterModelDao")
public class PlayerRegisterModelDaoImpl implements IPlayerRegisterModelDao{
	private static final Log log = LogFactory.getLog(PlayerRegisterModelDaoImpl.class);

    @Autowired
    private HibernateTemplate template;

	@Override
	public void createBean(PlayerRegisterModel playerRegisterModel) throws BeanAreadyException{
		if (log.isDebugEnabled()){
			log.debug("createBean:"+playerRegisterModel);
		}
		try{
			findById(playerRegisterModel.getId());
		}catch(NotFoundBeanException e){
            template.save(playerRegisterModel);
		}		
	}
	...
	public void updateBean(PlayerRegisterModel playerRegisterModel) throws NotFoundBeanException{
		...
		findById(playerRegisterModel.getId());
        template.update(playerRegisterModel);
	}
	...
	public void removeBean(PlayerRegisterModel playerRegisterModel) throws NotFoundBeanException{
		...
		findById(playerRegisterModel.getId());
        template.delete(playerRegisterModel);
	}
	...
	public void removeBean(Serializable id) throws NotFoundBeanException{
        ...
        PlayerRegisterModel playerRegisterModel = findById(id);
        template.delete(playerRegisterModel);
	}
	...
	public PlayerRegisterModel findById(Serializable id) throws NotFoundBeanException{
		...
		PlayerRegisterModel playerRegisterModel = (PlayerRegisterModel)template.get(PlayerRegisterModel.class, id);;
		if (null==playerRegisterModel)
	...
	@Override
	public List<PlayerRegisterModel> findByNameQuery(String nameQuery,Map<String,Object> params){
		if (log.isDebugEnabled()){
        ...
        List <PlayerRegisterModel> list = (List<PlayerRegisterModel>)template.findByNamedParam(nameQuery, keys.toArray(new String[0]),values.toArray(new Object[0]));
        return list;
	}

                @SuppressWarnings("unchecked")
                @Override
                public List<PlayerRegisterModel> findPages(final String nameQuery, final Map<String, Object> params, final int begin, final int offset) {
                    if (log.isDebugEnabled()) {
                    log.debug("findPages:[begin=" + begin + ",offset=" + offset + ",nameQuery=" + nameQuery + "]");
                    }
                    List<PlayerRegisterModel> list = (List<PlayerRegisterModel>) template.execute(new HibernateCallback
                        <Object>() {
                            @Override
