 ...
 * @author fangyaoxia
 */
@Repository(value = "playerSaleActionModelDao")
public class PlayerSaleActionModelDaoImpl implements IPlayerSaleActionModelDao{
	private static final Log log = LogFactory.getLog(PlayerSaleActionModelDaoImpl.class);

    @Autowired
    private HibernateTemplate template;

	@Override
	public void createBean(PlayerSaleActionModel playerSaleActionModel) throws BeanAreadyException{
		if (log.isDebugEnabled()){
			log.debug("createBean:"+playerSaleActionModel);
		}
		try{
			findById(playerSaleActionModel.getId());
		}catch(NotFoundBeanException e){
            template.save(playerSaleActionModel);
		}		
	}
	...
	public void updateBean(PlayerSaleActionModel playerSaleActionModel) throws NotFoundBeanException{
		...
		findById(playerSaleActionModel.getId());
        template.update(playerSaleActionModel);
	}
	...
	public void removeBean(PlayerSaleActionModel playerSaleActionModel) throws NotFoundBeanException{
		...
		findById(playerSaleActionModel.getId());
        template.delete(playerSaleActionModel);
	}
	...
	public void removeBean(Serializable id) throws NotFoundBeanException{
        ...
        PlayerSaleActionModel playerSaleActionModel = findById(id);
        template.delete(playerSaleActionModel);
	}
	...
	public PlayerSaleActionModel findById(Serializable id) throws NotFoundBeanException{
		...
		PlayerSaleActionModel playerSaleActionModel = (PlayerSaleActionModel)template.get(PlayerSaleActionModel.class, id);;
		if (null==playerSaleActionModel)
	...
	@Override
	public List<PlayerSaleActionModel> findByNameQuery(String nameQuery,Map<String,Object> params){
		if (log.isDebugEnabled()){
        ...
        List <PlayerSaleActionModel> list = (List<PlayerSaleActionModel>)template.findByNamedParam(nameQuery, keys.toArray(new String[0]),values.toArray(new Object[0]));
        return list;
	}

                @SuppressWarnings("unchecked")
                @Override
                public List<PlayerSaleActionModel> findPages(final String nameQuery, final Map<String, Object> params, final int begin, final int offset) {
                    if (log.isDebugEnabled()) {
                    log.debug("findPages:[begin=" + begin + ",offset=" + offset + ",nameQuery=" + nameQuery + "]");
                    }
                    List<PlayerSaleActionModel> list = (List<PlayerSaleActionModel>) template.execute(new HibernateCallback
                        <Object>() {
                            @Override
