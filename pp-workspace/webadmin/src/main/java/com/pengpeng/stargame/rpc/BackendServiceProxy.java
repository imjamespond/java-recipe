...
import com.pengpeng.stargame.managed.*;
...
//    @PostConstruct
    ...
    public List<IBackendService> getAllLogicServer(){
        try {
            List<NodeInfo> nodeInfoList=manageService.getServerNodes(ServerType.LOGIC);
            List<IBackendService>  backendServiceList=new ArrayList<IBackendService>();
            for(NodeInfo nodeInfo:nodeInfoList){
                RmiProxyFactoryBean proxy = new RmiProxyFactoryBean();
                url = String.format("rmi://%s:%d/%s",nodeInfo.getHost(),nodeInfo.getPort(),"logic");
                proxy.setServiceUrl(url);
                proxy.setServiceInterface(IBackendService.class);
                proxy.setRefreshStubOnConnectFailure(true);
                proxy.setCacheStub(true);
                proxy.afterPropertiesSet();
                proxy.prepare();
                IBackendService logicService = (IBackendService)proxy.getObject();
                backendServiceList.add(logicService);
                logger.info("Server node: " + nodeInfo.getHost() + ":" + nodeInfo.getPort() + " - alias:"+"logic");
            }
