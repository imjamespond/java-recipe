...
public class UserController implements GeneralController {
	...
	public List<RecieveVO> recieve(VoidRequest req) throws GeneralException {
		User user = userService.getRequestUser(); 
		user.recieve = System.currentTimeMillis();
	...
	public HashMap<String, ChatReqInvite> bufuMap = new HashMap<String, ChatReqInvite>();
	@HandlerAnno(name = "不服", cmd = "GameBufu", req = ChatReqInvite.class)
	public void bufu(ChatReqInvite req) throws GeneralException {
		User user = userService.getRequestUser();
		
		System.arraycopy(req.getRoles(), 0, user.roles, 0,
                Math.min(req.getRoles().length, user.roles.length));
		
		ChatReqInvite bufu = bufuMap.remove(req.getKey());
		if(null != bufu && bufu instanceof ChatReqInvite){
			gameService.accept(user, bufu.getUid());
			return;
		}
		throw new GeneralException(0, "bufu.expired");
	}
	//FIXME clear map in some time 
	public HashMap<String, ChatReqFangPiao> redeemMap = new HashMap<String, ChatReqFangPiao>();
