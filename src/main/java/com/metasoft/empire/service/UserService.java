...
public class UserService {
	...
	@Value("${user.persist.interval}")
	private int userPersistInterval;
	...
	public User getUserBySessionId(Integer sessionId) {
		BaseConnection<?> conn = connectionService.getConnBySession(sessionId);
	...
	public User onSignIn(User user) throws GeneralException {
		...
		if (currentUser != null) {
			onSignout(currentUser, currentUser.getConn().getSession());
		}
		...
		if (duplicateUser != null) {
			...
			if (preConn != null) {
				if (preConn.getSession() != conn.getSession()) {
					logger.info("multiple signin uid:{}, session:{}", preConn.getUserId(), preConn.getSession());
	...
	public void onSignout(User user,Integer sessionId) {
		if (null != user) {
			...
			if(sessionId != null && user.getConn()!=null){
				if (sessionId == user.getConn().getSession()) {
					user.setConn(null);
					//user.setGroup(null);//断线重进房需要
					userMap.remove(user.getId());
					userOfflineMap.put(user.getId(), user);
				}else{
					logger.warn("session not match current:{}, session:{}", sessionId, user.getConn().getSession());
				}
			}else{
