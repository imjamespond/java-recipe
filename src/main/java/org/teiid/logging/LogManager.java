...
public final class LogManager {
     ...
     * not logged if this parameter is null
     */
//    static final org.slf4j.Logger MyLog = LoggerFactory.getLogger(LogManager.class);
...
//        logMessage(MessageLevel.TRACE, context, msgParts);
        Utils.PrintCaller((fileName, lineNum, methodName, thread)->{
            System.out.printf("TRACE %s, [%s]%s(%d).%s %s\n", context, thread, fileName, lineNum, methodName, Arrays.deepToString(msgParts));
        });
    }
...
//        logMessage(MessageLevel.TRACE, context, msgPart);
        Utils.PrintCaller((fileName, lineNum, methodName, thread)->{
            System.out.printf("TRACE %s, [%s]%s(%d).%s %s\n", context, thread, fileName, lineNum, methodName, msgPart);
        });
    }
...
//        logMessage(MessageLevel.TRACE, context, msgPart, msgPart1, msgPart2);
        Object[] objs = {"TRACE", context, msgPart, msgPart1, msgPart2};
        Utils.PrintCaller(Arrays.deepToString(objs), 3);
    }
...
//        logMessage(MessageLevel.TRACE,context,e,msgParts);
        Object[] objs = {"TRACE", context, msgParts};
        Utils.PrintCaller(Arrays.deepToString(objs), 3);
    }
