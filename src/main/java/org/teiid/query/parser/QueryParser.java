...
public class QueryParser implements Parser {
    ...
    private SQLParser getSqlParser(Reader _sql) {
        String sqlstr = Utils.Reader2String(_sql);
        Reader sql = new StringReader(sqlstr);

        Utils.PrintCaller((file, line, method, thread)->{
            //System.out.printf("********getSqlParser: %s, caller: %s-%s(%d).%s\n", sqlstr, thread,file,line,method);
        });
