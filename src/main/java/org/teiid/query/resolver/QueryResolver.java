...
public class QueryResolver {
    ...
    public static TempMetadataStore resolveCommand(Command currentCommand, QueryMetadataInterface metadata, boolean resolveNullLiterals)
        ...
        Utils.PrintCaller((fileName, lineNum, methodName, thread)->{ System.out.printf("resolveCommand [%s]%s(%d).%s \n", thread, fileName, lineNum, methodName);});

        LogManager.logTrace(org.teiid.logging.LogConstants.CTX_QUERY_RESOLVER, new Object[]{"Resolving command", currentCommand, 4}); //$NON-NLS-1$
