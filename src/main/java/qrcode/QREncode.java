...
public class QREncode {
     ...
     * 生成二维码
     */
    public static void Encode(String content, String sourceFile, String targetFile) throws WriterException, IOException {
        int width = 200; // 图像宽度
        int height = 200; // 图像高度
        ...
        MatrixToImageWriter.writeToPath(bitMatrix, format, new File(targetFile).toPath());// 输出原图片
         ...
         */
        BufferedImage bufferedImage = LogoMatrix(MatrixToImageWriter.toBufferedImage(bitMatrix, matrixToImageConfig), new File(sourceFile));
//        BufferedImage bufferedImage = LogoMatrix(toBufferedImage(bitMatrix), new File("D:\\logo.png"));
        ImageIO.write(bufferedImage, format, new File(targetFile));//输出带logo图片
